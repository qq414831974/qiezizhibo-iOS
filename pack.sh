#/bin/bash: bin/bash: No such file or directoryuthor:Bruce http://www.heyuan110.com
...
projectName2=`find . -name *.xcworkspace | awk -F "[/.]" '{print $(NF-1)}'` #项目名称


if [ "$projectName" = "$projectName2" ]; then
isWorkSpace=true
else
isWorkSpace=false
fi
...
#rm -rf $buildAppToDir #是否删除文件夹
#rm -rf $projectDir/tmp #是否删除文件夹
